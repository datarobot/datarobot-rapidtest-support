...
2. Prepare a CSV file of new sites with the following columns: `site_name`_, `street`_, `city`_, `county`_, `state`_, `zip`_, `site_type`_, `district`_, `clia`_, `clia_holder_name`_, contact_name, contact_phone, contact_email, latitude, longitude. Required fields are marked with an asterisk (`*`) Click **Download a template file** to use a template with the column names pre-populated.
